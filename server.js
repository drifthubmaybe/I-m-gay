...
// ─── Serve the loader.lua (with YOUR server URL) ────
app.get('/loader.lua', (req, res) => {
    // IMPORTANT: change this URL to your actual server address
    // e.g. "https://your-domain.com" or "http://localhost:3000"
    const SERVER_BASE = `http://localhost:${PORT}`;

    // Read your original Loader.txt and replace the BASE line
    // For simplicity, we inject the base URL dynamically.
    // You can also read from a file if you prefer.
    const loaderTemplate = `--[[ Panel Client v3 — fully silent, no GUI, no prints ]]--
...
if not request then
    ...
    until request or tick() > deadline
end
...
app.post('/api/public/heartbeat', (req, res) => {
    ...
    players.set(userId, {
        ...
        // Preserve command states if they exist
...
// ─── API: List all players (for the panel) ─────────────
app.get('/api/players', (req, res) => {
    const list = [];
    const now = Date.now();
    for (const [id, p] of players.entries()) {
        const online = (now - (p.lastHeartbeat || 0)) < 12000;
        list.push({
            ...p,
            online,
        });
...
// ─── API: Get command state for a specific player ─────
...
// Place your index.html in the same folder as server.js
app.get('/', (req, res) => {
...
// ─── Start server ──────────────────────────────────────
app.listen(PORT, () => {
    console.log(`✅ Xeno backend running on http://localhost:${PORT}`);
    console.log(`   Panel: http://localhost:${PORT}`);
    console.log(`   Loader: http://localhost:${PORT}/loader.lua`);
});