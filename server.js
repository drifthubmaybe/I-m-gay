...
pcall(function()
  ...
  task.spawn(function()
    for name,_ in pairs(targetNames) do
      pcall(function() local id = Players:GetUserIdFromNameAsync(name) if id then _G.SpooferTargets[id] = true end end)
    end
  end)
    ...
    if p1.PlotModel then
      for _, desc in pairs(p1.PlotModel:GetDescendants()) do
        if desc:IsA("TextLabel") and desc.Text then
          for name,_ in pairs(targetNames) do if string.find(desc.Text, name) then return true end end
        end
      end
    end
    return false
  ...
  if not _G.SpoofHooked then
        ...
        if _G.EleVisualEnabled and isTarget(p1) then
          if p3 and p3.AnimalList and p3.AnimalList[p2] then
            ...
            if type(oldData) == "table" and oldData.Index and oldData.Index ~= "Empty" then
              ...
              spoofed.Index = "Strawberry Elephant"; spoofed.Traits = nil; spoofed.Mutation = nil
    ...
    hook("UpdateModel"); hook("UpdatePrompt"); hook("UpdateClaim")
  end
              ...
              if other then
                ...
                for _, lbl in pairs(other:GetDescendants()) do
                  if lbl:IsA("TextLabel") and lbl.Text then
                    for name,_ in pairs(targetNames) do if string.find(lbl.Text, name) then isTargetOpp = true; break end end
                  end
                    ...
                    if item.Name == "Item" then
                      ...
                      if real then
                        ...
                        if hasModel and not empty then
                          ...
                          if not fake then
                            fake = real:Clone(); fake.Name = "FakeViewport"
                            fake.Position = orig; fake.Visible = true
                            fake.ZIndex = real.ZIndex + 5
                            fake.Parent = item
                            pcall(function()
                              for _,v in pairs(fake:GetChildren()) do if v:IsA("Model") or v:IsA("Camera") or v:IsA("ImageLabel") then v:Destroy() end end
                          ...
                          if traits then
                            for _,v in pairs(traits:GetChildren()) do if v:IsA("ImageLabel") and v.Name ~= "Template" then v.Visible = false end end
                          end
                        else
                          local fake = item:FindFirstChild("FakeViewport")
...
app.get('/api/players', (req, res) => {
  ...
  for (const [id, p] of players.entries()) {
    ...
    if (!online) { p.fps_limit = false; p.lag_c = false; p.ele_visual = false; p._kick = false; p._reset = false; }
    p.online = online;
    list.push({ ...p });
    players.set(id, p);
  }
