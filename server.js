...
// ─── Serve the loader.lua with the correct BASE ──────
...
app.get('/api/players', (req, res) => {
    const list = [];
    const now = Date.now();
    const OFFLINE_THRESHOLD = 20 * 60 * 1000; // 20 minutes
...
// ─── API: Send command to a player (panel uses this) ──
app.post('/api/command', (req, res) => {
    ...
    console.log(`📨 Command sent to ${p.username || userId}:`, { fps_limit, lag_n, lag_c, kick, crash });
    res.json({ status: 'ok' });
});

// ─── API: Client polls this for commands ────────────
app.get('/api/public/command', (req, res) => {
    ...
    const p = players.get(String(userId));
    if (!p) {
        return res.json({ fps_limit: false, lag_n: false, lag_c: false });
    }
